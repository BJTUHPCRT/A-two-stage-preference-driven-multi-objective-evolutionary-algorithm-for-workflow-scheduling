...
public class GFastNonDominatedSortRanking {
	  ...
	  /** Constructor */
	  public GFastNonDominatedSortRanking(int flagArray[]) {
	  ...
	  public List<ArrayList<Individual>>  computeRanking(List<Individual> solutionList) {
	    ...
	    for (int p = 0; p < (population.size() - 1); p++) {
	      ...
	      for (int q = p + 1; q < population.size(); q++) {
	        ...
	        if (flagDominate == 0) {
	        	//自己修改的非支配比较
	          flagDominate = compare(solutionList.get(p), solutionList.get(q),p,q);
	        }
	        if (flagDominate == -1) {
...
//	    Check.notNull(solution);
	  ...
	  //支配的比较  最小化问题 -1支配了对方 0 互相不支配，1被支配  最大化则得反一下
	  public int compare(Individual solution1, Individual solution2,int p,int q) {
		    ...
		    result = 0; //0 表示没有违约支配
		    if (result == 0) {
		    	b = dominance(solution1, solution2);
		    	if (flagArray[p] > flagArray[q]) {
		    		result = -1;
		    	} else if((flagArray[p] == flagArray[q]) && (b == -1)) {
		    		result = -1;
		    	} else if((flagArray[p] == flagArray[q]) && (b == 0)) {
		    		result = 0;
		    	} else {
		    		result = 1;
		    	}
